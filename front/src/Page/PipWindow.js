import React, {
  ...
  import { PiPContext } from "../App";
  // Inside the Screen component, add:
  
  
  function PipWindow({roomId, meetingLink, hostUserId, status }) {
    ...
    const singleSchedule = allschedule.find(schedule => schedule.meetingLink === meetingLink);
    ...
    const {
      ...
      systemMessages,
      muteAllUsers
    } = useSocket(userId, roomId, userName, hostUserId);
  ...
  // }
    ...
    // Replace the togglePictureInPicture function with:
  const togglePictureInPicture = () => {
    dispatch(setPipWindow(false));
  };
  
    // const WindowContents = ({isPiP ,pipWindow ,hand, oncamera, offcamera, onmicrophone, offmicrophone, socket, roomId, localVideoRef, setVideoRef, toggleAudio, toggleVideo, toggleScreenShare, toggleRecording, endMeeting}) => {
    //   // Create new refs for PiP window
    //   const pipLocalVideoRef = useRef(null);
    //   const pipVideoRefs = useRef({});
    //   const { isHandRaised, show, mainSectionMargin, showEmojis,  } = useSelector((state) => state.meeting);
    //     // Get other necessary stat
    ...
    //   const handlePiPStateChange = (action) => {
    //     dispatch(action);
    //   };
    ...
    //       // Create a new store instance for the PiP window
    //   const pipStore = configureStore().store;
  
    //   console.log("pipWindow", pipStore);
      
  
    //     const pipRoot = ReactDOM.createRoot(dpip.document.getElementById("pip-root"));
    //     pipRoot.render(
    //       <Provider store={pipStore}>
    //         <WindowContents  
    //         isPiP={true}
    //         pipWindow={dpip}
    //         hand={hand}
    ...
    //       pipRoot.unmount();
    //       dispatch(setPipWindow(false)); // Update Redux state when PiP closes
    //     });
